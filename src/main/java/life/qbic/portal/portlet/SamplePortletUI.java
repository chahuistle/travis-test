...
public class SamplePortletUI extends QBiCPortletUI {

    private static final Logger LOG = LogManager.getLogger(SamplePortletUI.class);
    ...
    private static final boolean USING_OPEN_BIS =false;
    ...
    private Layout REMOVE_THIS_METHOD_AND_DO_YOUR_OWN_THING_COMMA_PLEASE(final VaadinRequest request) {
        ...
        builder.append("<br/>You can now start developing. Start by modifying the <font face='monospace'>getPortletContent</font> method in the generated <font face='monospace'>src/main/java/life/qbic/portal/portlet/SamplePortletUI.java</font> file.<br/><br/>");
        ...
        final Panel mainPanel = new Panel("Sample Portlet");
    ...
    private void insertOpenBISConnectionTesterComponents(final Layout layout) {
        ...
        openBisTestButton.addClickListener((Button.ClickListener) event -> {
            if (isOpenBISWorking(dataSourceUser.getValue(), dataSourcePassword.getValue(), dataSourceUrl.getValue())) {
                Styles.notification("Sample Portlet", "Connection to openBIS works!", NotificationType.SUCCESS); 
            } else {
                Styles.notification("Sample Portlet", "Could not establish openBIS connection. Check the logs for exceptions.", NotificationType.ERROR); 
            }
        });
    ...
    private void insertInternalDatabasesConnectionTesterComponents(final Layout layout) {
        ...
        internalDatabaseTestButton.addClickListener((Button.ClickListener) event -> {
            if (isQBiCDatabaseWorking(username.getValue(), password.getValue(), jdbcUrl.getValue())) {
                Styles.notification("Sample Portlet", "Connection to QBiC database works!", NotificationType.SUCCESS); 
            } else {
                Styles.notification("Sample Portlet", "Could not establish connection to QBiC database. Check the logs for exceptions.", NotificationType.ERROR); 
            }
        });
